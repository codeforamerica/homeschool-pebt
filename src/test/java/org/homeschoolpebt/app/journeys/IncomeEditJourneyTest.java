...
public class IncomeEditJourneyTest extends AbstractBasePageTest {
  @MockBean
  ...
  void test() {
    ...
    // Add a job as semimonthly income, then change it to hourly after the review screen
    ...
    testPage.clickContinue();
    testPage.findElementById("incomeWillBeLess-false-label").click(); // Will income be less?
    testPage.clickButton("Continue");
    assertPageTitle("Great! Any other jobs in the household to add?");
    testPage.clickLink("edit job");
    assertPageTitle("Who do you want to add the job for?");
    testPage.findElementByCssSelector("input[type=radio]").click(); // Click first radio button
    testPage.clickContinue();
    assertPageTitle("Add a job for Testy McTesterson");
